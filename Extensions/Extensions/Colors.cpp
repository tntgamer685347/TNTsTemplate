...
Color& Color::FromHex(std::string hexColor)
{
	...
	uint32_t decimalColor = 0xFFFFFF;
	uint32_t alphaColor = 255;
	...
	if (Format::IsStringHexadecimal(hexColor))
	{
		if (hexColor.length() == 8)
		{
			alphaColor = Format::ToDecimal(hexColor.substr(6, 2)); // Optional, if an alpha value is provided.
		}
		...
		decimalColor = Format::ToDecimal(hexColor);

		if (decimalColor > 0xFFFFFF)
		{
			decimalColor = 0xFFFFFF;
		}

		FromDecimal(decimalColor);
		A = alphaColor;
	}
...
Color& Color::Cycle(int32_t step)
{
	if ((R == 255) && (G < 255) && (B == 0)) { G += step; } 		// Green goes up to 255.
	else if ((R > 0) && (G == 255) && (B == 0)) { R -= step; }		// Red goes down to 0.
	else if ((R == 0) && (G == 255) && (B < 255)) { B += step; }	// Blue goes up to 255.
	else if ((R == 0) && G > 0 && (B == 255)) { G -= step; }		// Green goes down to 0.
	else if ((R < 255) && (G == 0) && (B == 255)) { R += step; }	// Red goes up to 255.
	else if ((R == 255) && (G == 0) && (B > 0)) { B -= step; }		// Blue goes down to 0.
	return *this;
}
...
LinearColor& LinearColor::Cycle(int32_t step)
{
	return FromColor(ToColor().Cycle(step));
}
